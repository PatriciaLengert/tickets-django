...
def getTickets():
    sql = f"""select  
                date_format(t.data_inicio, '%d/%m/%Y %H:%i') as fdata_inicio,
                date_format(t.data_fim, '%d/%m/%Y %H:%i') as fdata_fim,
                t.* 
            from tickets t order by cod_ticket desc
            """
...
# ------------INSERT---------------
def setTicket(
    cod_ticket: int, desc_problema, cpf: str, data_inicio, data_fim: datetime
):
